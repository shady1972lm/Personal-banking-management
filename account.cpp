...
double Account::total = 0; //存款总金额全局变量(所有账户）
RecordMap Account::recordMap; // Account类中的账户记录
...
void Account::record(const Date& date, double amount, const string& desc){
    ...
    AccountRecord record(date, this, amount, balance, desc); //将添加的数据存入账目记录类
    recordMap.insert(make_pair(date, record)); //将数据插入到关联容器内部,date为主键 
    record.show();
}
...
void SavingsAccount::deposit(const Date& date, double amount, const string& desc){
    ...
    acc.change(date, getBalance()); //调用accumulator函数，更新值，变更当前的时间，还有值
}
...
void SavingsAccount::settle(const Date& date){
    ...
    if (interest != 0)
        record(date, interest, "interest");//将利息记账
    acc.reset(date, getBalance());//初始化，日期，数值，累加器清零
}
